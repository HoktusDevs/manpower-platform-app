...
import type { TableColumn, TableAction } from '../../components/UniversalTable/UniversalTableManager';
...
export const EnhancedFormsManager: React.FC = () => {
  ...
  const tableColumns: TableColumn<Form>[] = [
    ...
    },
    {
      ...
      label: 'Respuestas',
      render: (form) => (
        <span className="text-sm text-gray-500">{form.currentSubmissions || 0} respuestas</span>
      )
    },
    {
      key: 'createdAt',
  ...
  const rowActions: TableAction<Form>[] = [
    {
      ...
      label: 'Publicar',
      variant: 'primary',
      onClick: (form) => handlePublishForm(form.formId),
      show: (form) => form.status === 'DRAFT'
    },
    {
      key: 'pause',
      label: 'Pausar',
      variant: 'secondary',
      onClick: (form) => handlePauseForm(form.formId),
      show: (form) => form.status === 'PUBLISHED'
    },
    {
      key: 'edit',
      label: 'Editar',
    ...
    },
    {
      ...
      label: 'Eliminar',
      variant: 'danger',
      onClick: (form) => handleDeleteForm(form.formId)
    }
  ];

  // Filtrar formularios basándose en el término de búsqueda
  const filteredForms = forms.filter(form => 
    form.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (form.description && form.description.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  // Configuración de acciones en lote
  const bulkActions = [
    {
      key: 'create',
    ...
    },
    {
      ...
      onClick: async (selectedForms: Form[]) => {
        if (selectedForms.length === 0) {
          alert('Por favor selecciona al menos un formulario para eliminar');
          return;
        }
        if (confirm(`¿Estás seguro de que quieres eliminar ${selectedForms.length} formulario(s)?`)) {
